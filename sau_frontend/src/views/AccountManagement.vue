            ...
            <div v-if="filteredAccounts.length > 0" class="account-list">
              <el-table :data="filteredAccounts" style="width: 100%">
                ...
                <el-table-column prop="status" label="状态">
                  <template #default="scope">
                    <el-tooltip
                      v-if="isStatusClickable(scope.row.status)"
                      content="点击重新登录"
                      placement="top"
                    >
                      <el-tag
                ...
                <el-table-column label="操作">
                  <template #default="scope">
                    <el-button size="small" type="warning" :icon="Refresh" @click="handleReLogin(scope.row)">重新登录</el-button>
        ...
        <div v-if="sseConnecting" class="qrcode-container">
          <div v-if="qrCodeData && !loginStatus" class="qrcode-wrapper">
            ...
            <el-button
              type="success"
              size="large"
              :loading="scanConfirmed"
              :disabled="scanConfirmed"
              class="manual-confirm-btn"
              @click="confirmScan"
            >
              {{ scanConfirmed ? '等待中...' : '我已扫码登录' }}
            </el-button>
          </div>
          <div v-else-if="loginPromptMessage && !loginStatus" class="qrcode-wrapper">
            <p class="qrcode-tip">{{ loginPromptMessage }}</p>
            <el-button
              type="success"
              size="large"
              :loading="scanConfirmed"
...
const scanConfirmed = ref(false) // 用户手动点击"我已扫码登录"
...
const connectSSE = (platform, name) => {
  ...
  eventSource.onmessage = (event) => {
    ...
    let data = rawData
    console.log(`[SSE] 收到消息, 长度=${rawData.length}, 前50字符=${rawData.substring(0, 50)}`)

    if (rawData.startsWith('{') || rawData.startsWith('[')) {
      try {
        const parsed = JSON.parse(rawData)
        data = parsed?.image_data_url || parsed?.qrcode?.image_data_url || rawData
      } catch (error) {
        console.warn('[SSE] JSON 消息解析失败，按原始字符串处理:', error)
      }
    }

    if (data.startsWith('PROMPT:')) {
      loginPromptMessage.value = data.substring(7)
      return
    }
    ...
    // 如果还没有二维码数据，判断是否是二维码（base64 或 URL）
    if (!qrCodeData.value && (data.length > 100 || data.startsWith('http'))) {
      try {
        ...
        console.log('[SSE] 二维码已设置')
      } catch (error) {
        console.error('[SSE] 处理二维码数据出错:', error)
      }
    }
    // 如果收到状态码
    else if (data === '200' || data === '500') {
      console.log(`[SSE] 收到状态码: ${data}`)
  ...
  eventSource.onerror = (error) => {
    console.error('[SSE] 连接错误 (readyState=' + eventSource.readyState + '):', error?.message || error)
...
const confirmScan = async () => {
  if (scanConfirmed.value) return
  scanConfirmed.value = true
  const baseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5409'
  try {
    const resp = await fetch(`${baseUrl}/confirmScan?id=${encodeURIComponent(accountForm.name)}`)
    const data = await resp.json()
    if (data.code === 200) {
      console.log('[SSE] 已发送手动确认信号')
    } else {
      console.warn('[SSE] 手动确认失败:', data.msg)
      scanConfirmed.value = false
      ElMessage.error('确认失败，请重试')
    }
  } catch (e) {
    console.error('[SSE] 手动确认请求失败:', e)
    scanConfirmed.value = false
