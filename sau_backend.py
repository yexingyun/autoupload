...
from uploader.tencent_uploader.main import get_tencent_cookie as uploader_get_tencent_cookie
...
def upsert_account(conn, type_val, file_path, user_name):
    cursor = conn.cursor()
    cursor.execute(
        '''
...
def build_web_login_launch_kwargs():
    launch_kwargs = {
        "headless": False,
        "args": [
...
@app.route('/login')
def login():
    # 1 小红书 2 视频号 3 抖音 4 快手 5 百家号 6 TikTok 7 Bilibili
...
@app.route('/confirmScan', methods=['GET'])
def confirm_scan():
    account_id = request.args.get('id')
    if not account_id:
        return jsonify({
    ...
    if account_id not in active_confirmations:
        return jsonify({
            "code": 404,
            "msg": "当前没有进行中的登录会话",
            "data": None
    ...
    active_confirmations[account_id] = True
    return jsonify({
        "code": 200,
        "msg": "确认信号已收到",
        "data": None
...
def run_async_function(type,id,status_queue):
    match type:
        ...
        case '2':
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(login_tencent_with_uploader(id, status_queue))
            loop.close()
        case '3':
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
        ...
        case '5':
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(login_baijiahao_with_browser(id, status_queue, lambda: active_confirmations.get(id, False)))
            loop.close()
        case '6':
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(login_tiktok_with_browser(id, status_queue, lambda: active_confirmations.get(id, False)))
            loop.close()
        case '7':
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(login_bilibili_web_not_supported(status_queue))
            loop.close()


async def login_tencent_with_uploader(account_name, status_queue):
    cookie_filename = f"{uuid.uuid1()}.json"
    cookie_file = Path(BASE_DIR / "cookiesFile" / cookie_filename)
    cookie_file.parent.mkdir(parents=True, exist_ok=True)

    async def qrcode_callback(payload: dict):
        image_data_url = payload.get("image_data_url") if isinstance(payload, dict) else None
        if image_data_url:
            status_queue.put(image_data_url)

    try:
        result = await uploader_get_tencent_cookie(
            str(cookie_file),
            qrcode_callback=qrcode_callback,
            headless=False,
        )
        print(f"📋 视频号 uploader 登录结果: {result}")

        if result.get("success"):
            with sqlite3.connect(Path(BASE_DIR / "db" / "database.db")) as conn:
                upsert_account(conn, 2, cookie_filename, account_name)
        ...
        if cookie_file.exists():
            try:
                ...
                print(f"⚠️ 清理失败的视频号 cookie 文件失败: {exc}")

        error_message = result.get("message") or "视频号登录失败"
        ...
        if cookie_file.exists():
            try:
                ...
                pass
        print(f"❌ 视频号 uploader 登录异常: {exc}")
        status_queue.put(f"ERROR:{str(exc)}")
        status_queue.put("500")


async def login_with_manual_browser(account_name, status_queue, platform_type, login_url, cookie_auth_func, is_confirmed, platform_label):
    cookie_filename = f"{uuid.uuid1()}.json"
    cookie_file = Path(BASE_DIR / "cookiesFile" / cookie_filename)
    cookie_file.parent.mkdir(parents=True, exist_ok=True)

    status_queue.put(f"PROMPT:请在新打开的浏览器中完成{platform_label}登录，完成后回到弹窗点击“我已扫码登录”。")

    async with async_playwright() as playwright:
        browser = await playwright.chromium.launch(**build_web_login_launch_kwargs())
        ...
        try:
            confirmed = False
            for _ in range(240):
                if not confirmed and callable(is_confirmed) and is_confirmed():
                    confirmed = True
                    status_queue.put(f"PROMPT:已收到确认，正在校验{platform_label}登录状态...")

                if confirmed:
                    await context.storage_state(path=str(cookie_file))
                    if await cookie_auth_func(str(cookie_file)):
                        with sqlite3.connect(Path(BASE_DIR / "db" / "database.db")) as conn:
                            upsert_account(conn, platform_type, cookie_filename, account_name)
                        status_queue.put("200")
                        return
            ...
            if cookie_file.exists():
                cookie_file.unlink()
            status_queue.put(f"ERROR:{platform_label}登录超时或登录态校验失败，请确认浏览器中已登录成功后重试。")
            status_queue.put("500")
        finally:
            await context.close()
            await browser.close()
...
async def login_baijiahao_with_browser(account_name, status_queue, is_confirmed):
    await login_with_manual_browser(
        account_name,
        status_queue,
        5,
        "https://baijiahao.baidu.com/builder/theme/bjh/login",
        baijiahao_cookie_auth,
        is_confirmed,
        "百家号",
    )


async def login_tiktok_with_browser(account_name, status_queue, is_confirmed):
    await login_with_manual_browser(
        account_name,
        status_queue,
        6,
...
async def login_bilibili_web_not_supported(status_queue):
    status_queue.put("ERROR:Bilibili 网页端登录暂未接入，请先通过 CLI 完成登录或上传现有 Cookie 文件。")
    status_queue.put("500")
...
def sse_stream(account_id, status_queue):
    try:
        ...
        print(f"SSE连接已关闭: {account_id}")
    finally:
        print(f"清理登录会话: {account_id}")
        active_queues.pop(account_id, None)
        active_confirmations.pop(account_id, None)
